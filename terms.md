---
layout: default
title: User Agreement
description: User Agreement for Yuedu Reader.
---

<p><img src="{{ site.baseurl }}/Resources/Assets.xcassets/YueduLogo.imageset/YueduLogo.png" alt="Yuedu Reader logo" width="88" style="border-radius:18px"></p>

# 使用者協議

閱讀（Yuedu Reader）
最後更新：2026 年 6 月 30 日

[English](#en)

本協議適用於你下載、安裝、存取或使用閱讀（Yuedu Reader）及其相關功能。使用本 App 即表示你理解並同意本協議；若你不同意，請停止使用本 App。

## 1. 服務性質

閱讀是一款本機優先的閱讀器工具，用於管理、閱讀你自行匯入或自行設定來源取得的內容。除非另有明確說明，我們不提供、託管、上傳、銷售或分發任何第三方小說、書籍、漫畫、音訊、RSS 內容或其他受版權保護的內容。

## 2. 你的內容與書源責任

你需自行確保匯入、下載、閱讀、同步、分享或透過書源取得的內容具有合法權利或授權。你不得使用本 App 侵犯著作權、商標權、隱私權或其他合法權益。

書源、RSS、TTS、WebDAV、線上閱讀地址、Cookie、登入狀態與其他第三方服務均由你自行配置或選擇使用。你應遵守第三方網站、服務與內容提供者的條款。因第三方服務、第三方內容、書源規則或你自行配置造成的爭議、損失或法律責任，由你自行承擔。

## 3. 禁止行為

你不得使用本 App：

- 繞過付費牆、DRM、登入限制、存取控制或其他技術保護措施。
- 大量抓取、攻擊、干擾或濫用第三方網站、伺服器或 API。
- 儲存、傳播或協助取得侵權、違法、惡意或未授權內容。
- 逆向工程、修改、重新分發本 App，除非適用法律或開源授權允許。

## 4. 帳號、同步與資料

部分功能可能需要 Apple、Google、iCloud、WebDAV 或其他第三方服務。這些服務由其提供者管理，並受其各自條款與政策約束。請妥善保管你的帳號、Cookie、Token、伺服器地址與同步憑證。

## 5. 智慧財產權

本 App 的程式、介面、圖示、品牌、文件與原創素材由其權利人保留相關權利。你保留自己合法擁有內容的權利；本協議不會把你的書籍或個人內容轉讓給我們。

## 6. 免責聲明

本 App 按「現狀」提供。我們不保證第三方網站、書源、RSS、TTS、WebDAV、iCloud、登入服務或網路內容可用、準確、合法、穩定或持續相容。你需自行備份重要資料，並自行判斷第三方內容與服務的合法性、安全性與可靠性。

在法律允許範圍內，我們不對因使用或無法使用本 App、第三方服務、第三方內容、資料遺失、帳號問題、同步失敗、內容爭議或使用者自行配置造成的間接、附帶、特殊、懲罰性或衍生損害負責。

## 7. 條款變更

我們可能更新本協議。更新後會修改本頁「最後更新」日期；重大變更可能在 App 或專案頁面提示。你在更新後繼續使用，即表示接受更新後的協議。

## 8. 其他條款

如果本協議任何條款被認定無效，其餘條款仍繼續有效。付費功能另適用[付費服務條款]({{ site.baseurl }}/paid-terms.html)；個人資料處理另適用[隱私權政策]({{ site.baseurl }}/privacy.html)。

## 9. 聯絡我們

- Email: [r3212239269@gmail.com](mailto:r3212239269@gmail.com)
- GitHub: [github.com/CHANG-JUI-LIN/Yuedu-reader/issues](https://github.com/CHANG-JUI-LIN/Yuedu-reader/issues)

---

## English {#en}

# User Agreement

Yuedu Reader
Last updated: June 30, 2026

This agreement applies when you download, install, access, or use Yuedu Reader and related features. By using the app, you understand and agree to this agreement. If you do not agree, stop using the app.

## 1. Nature of the Service

Yuedu Reader is a local-first reader tool for managing and reading content that you import yourself or obtain through sources you configure. Unless expressly stated otherwise, we do not provide, host, upload, sell, or distribute third-party novels, books, comics, audio, RSS content, or other copyrighted content.

## 2. Your Content and Source Responsibility

You are responsible for ensuring that you have lawful rights or authorization for content you import, download, read, sync, share, or access through sources. You may not use the app to infringe copyright, trademark, privacy, or other legal rights.

Book sources, RSS, TTS, WebDAV, online reading URLs, cookies, login state, and other third-party services are configured or chosen by you. You must comply with the terms of third-party websites, services, and content providers. Disputes, losses, or legal responsibilities caused by third-party services, third-party content, source rules, or your own configuration are your responsibility.

## 3. Prohibited Uses

You may not use the app to:

- Bypass paywalls, DRM, login restrictions, access controls, or other technical protection measures.
- Scrape at scale, attack, interfere with, or abuse third-party websites, servers, or APIs.
- Store, distribute, or assist access to infringing, illegal, malicious, or unauthorized content.
- Reverse engineer, modify, or redistribute the app except as allowed by applicable law or open-source licenses.

## 4. Accounts, Sync, and Data

Some features may require Apple, Google, iCloud, WebDAV, or other third-party services. These services are managed by their providers and governed by their own terms and policies. Keep your accounts, cookies, tokens, server URLs, and sync credentials secure.

## 5. Intellectual Property

The app's software, interface, icons, brand, documentation, and original materials remain owned by their respective rights holders. You keep your rights in content you lawfully own. This agreement does not transfer your books or personal content to us.

## 6. Disclaimer

The app is provided "as is." We do not guarantee that third-party websites, book sources, RSS, TTS, WebDAV, iCloud, sign-in services, or network content will be available, accurate, lawful, stable, or continuously compatible. Back up important data and evaluate the legality, safety, and reliability of third-party content and services yourself.

To the maximum extent permitted by law, we are not liable for indirect, incidental, special, punitive, or consequential damages caused by use of or inability to use the app, third-party services, third-party content, data loss, account issues, sync failures, content disputes, or user configuration.

## 7. Changes

We may update this agreement. Updates will change the "Last updated" date on this page; material changes may also be noted in the app or on the project page. Continued use after an update means you accept the updated agreement.

## 8. Other Terms

If any provision is found invalid, the remaining provisions remain effective. Paid features are also governed by the [Paid Service Terms]({{ site.baseurl }}/paid-terms.html); personal data handling is governed by the [Privacy Policy]({{ site.baseurl }}/privacy.html).

## 9. Contact

- Email: [r3212239269@gmail.com](mailto:r3212239269@gmail.com)
- GitHub: [github.com/CHANG-JUI-LIN/Yuedu-reader/issues](https://github.com/CHANG-JUI-LIN/Yuedu-reader/issues)
