...
struct ComputedStyleTreeTests {
    func parseBody(_ html: String) async -> Element? {
        await HTMLBuilderDOMParser().parse(
            html: html,
            collectStyles: { _ in [] },
            stylesheetCache: nil
        )?.body
...
struct BlockLayoutTests {
    ...
    @Test func stacksChildrenVertically() {
        ...
        #expect(childA.frame.rawValue == CGRect(x: 0, y: 0, width: 200, height: 0))
        #expect(childB.frame.rawValue == CGRect(x: 0, y: 0, width: 200, height: 0))
...
struct DisplayListTests {
    @Test func convertsFragmentsToItems() {
        let page = PageFragments(
            index: 0,
            pageRect: PageLocalRect(rawValue: CGRect(x: 0, y: 0, width: 300, height: 100)),
            fragments: [
                .fill(FillFragment(rect: PageLocalRect(rawValue: CGRect(x: 10, y: 0, width: 280, height: 40)),
                                   documentRect: DocumentRect(rawValue: CGRect(x: 10, y: 0, width: 280, height: 40)),
                                   color: .red,
                                   cornerRadius: 0,
                ...
                .text(TextFragment(sourceRange: NSRange(location: 0, length: 5), nodeID: 2,
                                   ...
                                   rect: PageLocalRect(rawValue: CGRect(x: 10, y: 4, width: 60, height: 20)),
                                   documentRect: DocumentRect(rawValue: CGRect(x: 10, y: 4, width: 60, height: 20)),
                                   baselineY: 20, font: .systemFont(ofSize: 16), color: .black,
                                   ctLine: nil)),
                .group([.fill(FillFragment(rect: PageLocalRect(rawValue: CGRect(x: 5, y: 50, width: 100, height: 10)),
                                           documentRect: DocumentRect(rawValue: CGRect(x: 5, y: 50, width: 100, height: 10)),
                                           color: .blue,
                                           cornerRadius: 2,
                                           borderTop: .zero, borderBottom: .zero, borderLeft: .zero, borderRight: .zero,
        ...
        #expect(text.rect.rawValue == CGRect(x: 10, y: 4, width: 60, height: 20))
